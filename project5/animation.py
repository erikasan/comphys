...
plt.legend([r'Backward Euler $\Delta x = 1/10$', 'Analytical solution'], loc = 'upper left')
#plt.legend([r'Analytical solution'], loc = 'upper left')

animation = FuncAnimation(fig, func = animation_frame, frames = np.arange(1, len(data[0, :]), 2), interval = 1, repeat_delay = 1000)

animation.save('backwardeuler_dotted_dx=0.1.gif', writer='imagemagick', fps=20)
